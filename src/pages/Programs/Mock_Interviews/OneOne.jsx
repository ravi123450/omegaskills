...
export default function OneOne() {
  ...
  const bookingHref = formUrl && /^https?:\/\//i.test(formUrl) ? formUrl : undefined;
  ...
  const grouped = useMemo(() => {
    ...
    const order = ["DSA", "SYSTEM DESIGN", "BEHAVIORAL", "SOFT SKILLS", "GENERAL"];
    const out = new Map();
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <section className="mx-auto max-w-7xl px-4 pt-10 pb-6 md:px-6 md:pt-14">
        ...
        <p className="mt-4 max-w-2xl text-base md:text-lg text-slate-300">
          Practice real interview rounds with mentors. Get precise, actionable feedback and a plan to improve.
        </p>
              ...
              {!loading && slots.length > 0 && (
                <div className="mt-8 space-y-6">
                  {Array.from(grouped.entries()).map(([track, list]) => (
                    <SlotSection key={track} track={track} list={list} bookingHref={bookingHref} />
                  ))}
                </div>
              )}
            </div>
              ...
              <Card className="border-slate-800/70 bg-slate-900/50">
                <CardContent className="p-6">
                  ...
                  <ul className="space-y-2 text-sm text-slate-300/90">
                    {["30–40 min interview", "10–15 min feedback", "Action plan", "Optional re-book"].map(
                      (x) => (
...
function SlotSection({ track, list, bookingHref }) {
  return (
    <div className="rounded-2xl border border-slate-800/70 bg-slate-900/40 p-4">
      <div className="text-[11px] font-semibold tracking-wider text-slate-400">{track}</div>
...
/* FLEX-BASED CARD — no overlap, ever */
function SlotCard({ s, bookingHref }) {
  ...
  return (
    <div className="rounded-xl border border-slate-800/60 bg-gradient-to-br from-slate-900/70 to-slate-900/40 p-5 shadow-[0_0_0_1px_rgba(15,23,42,0.4)] hover:border-orange-600/40 transition-colors">
      {/* Wrapper prevents overlap */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        {/* LEFT: info stack */}
        <div className="min-w-0">
          {/* Time row */}
          <div className="inline-flex items-center gap-2 rounded-lg bg-slate-900/80 px-3 py-1 ring-1 ring-inset ring-slate-700">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-slate-400">
            <span className="rounded-full bg-slate-800/60 px-2 py-0.5">{s.mode || "Online"}</span>
            <span className="rounded-full bg-slate-800/60 px-2 py-0.5">
              {s.capacity} seat{s.capacity > 1 ? "s" : ""}
            </span>
            {s.notes ? (
              <span className="rounded-full bg-slate-800/60 px-2 py-0.5">{s.notes}</span>
            ) : null}
          </div>
        </div>
...
function BookButton({ bookingHref, big = false }) {
  ...
  if (disabled) {
    return (
      <Button
        className={(big ? "h-11 px-4" : "w-full") + " bg-orange-500/30 text-orange-200 cursor-not-allowed"}
        disabled
      >
  ...
  return (
    <Button
      asChild
      className={
        "bg-orange-500 text-slate-900 hover:bg-orange-400 hover:text-black " + (big ? "h-11 px-4" : "w-full")
      }
    >
      <a href={bookingHref} target="_blank" rel="noreferrer">
