...
import { Mail, Lock, ArrowRight, Eye, EyeOff, LifeBuoy } from "lucide-react";
...
export default function Login() {
  ...
  async function onSubmit(e) {
    ...
    setLoading(true);
    try {
      await login(email.trim(), password); // server sets cookie or returns token
      if (!localStorage.getItem("auth_token")) localStorage.setItem("auth_token", "session");
      window.dispatchEvent(new Event("auth-changed"));
      nav(from, { replace: true });
    } catch (e) {
      const msg = e?.message || "Login failed. Please try again.";
      setErr(/invalid/i.test(msg) ? "Invalid email or password. Please try again." : msg);
    } finally {
      setLoading(false);
    }
  }
        ...
        <Card className="border border-slate-800/60 bg-slate-900/40 backdrop-blur">
          <CardContent className="p-8">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <Badge className="bg-orange-600/20 text-orange-300">Welcome back</Badge>
              <span className="text-slate-300">to</span>
              <Badge className="bg-orange-600/20 text-orange-300">Ω Omega Skills Academy</Badge>
            </div>

            <h1 className="text-3xl md:text-4xl font-extrabold leading-tight">
              Log in to <span className="bg-gradient-to-r from-orange-400 to-amber-200 bg-clip-text text-transparent">Your Future</span>
            </h1>
            <p className="mt-3 text-sm text-slate-300">Access your dashboard, workshops, and resume tools.</p>
            ...
            <form onSubmit={onSubmit} className="mt-6 grid gap-5" noValidate>
              ...
              <label className="text-sm text-slate-300">
                ...
                <div className="mt-2 flex items-center gap-2 rounded-xl border border-slate-800/60 bg-slate-900/40 px-3 focus-within:border-orange-600/50">
                  ...
                  <input type={showPassword ? "text" : "password"} autoComplete="current-password" className="w-full bg-transparent py-3 text-slate-100 outline-none" value={password} onChange={(e) => setPassword(e.target.value)} required />
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                <Button
                  type="button"
                  variant="secondary"
                  className="inline-flex items-center gap-2 bg-slate-800 text-slate-100 hover:bg-slate-700"
                ...
                <Button type="submit" size="lg" disabled={loading} className="bg-orange-500 text-slate-900 hover:bg-orange-400 hover:text-black font-semibold cursor-pointer">
                  {loading ? "Logging in..." : "Log in"} {!loading && <ArrowRight className="ml-2 h-4 w-4" />}
                </Button>
              </div>
            </form>
            ...
            <div className="mt-6 flex items-center justify-between text-sm text-slate-300">
              <span>
                New here?{" "}
                <Link to="/signup" className="text-orange-300 hover:text-orange-200 underline">Create an account</Link>
              </span>

              <Link to="/forgot-password" className="text-orange-300 hover:text-orange-200 underline">Forgot password?</Link>
            </div>
          </CardContent>
        </Card>
      </motion.main>
